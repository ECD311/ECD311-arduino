...
void setup() {
   ...
   * The date and time can be changed via the 'SetDateTime' example that comes with the 'clock' library
   */
  //Set Pin Modes
  ...
  if (!accelComp.begin()) // with no arguments, this uses default addresses (AG:0x6B, M:0x1E) and i2c port (Wire).
  {
...
void loop() {
  ...
  TempAndHumid();
  Wind();
  Attitude(accelComp.ax, accelComp.ay, accelComp.az, accelComp.mx, accelComp.my, accelComp.mz);
  ...
  //ADD: May want to have the State 'persist' for a little bit, to prevent the solar panel from switching States too often
  //ADD: Maybe also have the State only be triggered after multiple checks, to avoid accidently putting it in a wrong mode
  ...
  switch(State){
      ...
      case 4: // Protect from Snow
        ...
        MoveSPElev(SnowElev);
        MoveSPAzi(SnowAzi);
         //Probably shouldn't trigger this the moment it snows- flurries are common in bing
...
/*
  ...
  We should be able to just use one accelerometer's pitch/roll/heading to determine the position of the solar panel.
  The proper pitch/roll/heading can only be determined once the accelerometer is in position on the solar panel
  Could add a check for the pitch/roll/heading right before hitting the limit switch, as an additional failsafe
*/
// Calculate elevation(aka altitude/pitch), roll, and azimuth(aka yaw/heading) of the solar panel
...
void Attitude(float ax, float ay, float az, float mx, float my, float mz)
{
  ...
  if (MeasuredAzimuth > PI) MeasuredAzimuth -= (2 * PI);
  else if (MeasuredAzimuth < -PI) MeasuredAzimuth += (2 * PI);
...
void MoveSPElev(float Elev){
  ...
  if(Elev-ElevRange >= MeasuredElevation){
    EnableMotor(2, 1);//Need to determine which direction leads to which angle change CHECK: if second variable should be 1 or 2
  }
...
void MoveSPAzi(float Azi){
  ...
  if(Azi-AziRange >= MeasuredAzimuth){
    EnableMotor(1, 1);//Need to determine which direction leads to which angle change
  }
...
void CheckLimitSwitches(){
  //See if near limit switch to avoid triggering it, figure out via measured vals
...
void ReceivePiData(){
  //ADD: Could try to get weather data to predict rain,snow, cloudy weather, azimuth, elevation
}
